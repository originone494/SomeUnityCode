...
namespace ARPG.Animation
{
    public class Mixer : AnimBehaviour
    {
        ...
        public Mixer(PlayableGraph graph) : base(graph)
        {
            ...
            m_adapterPlayable.AddInput(m_animMixer, 0, 0f);
        ...
        public override void Execute(Playable playable, FrameData info)
        {
            ...
            SetWeight(m_targetIndex, 1f);
            SetWeight(m_currentIndex, 0f);
            m_currentIndex = m_targetIndex;
            m_targetIndex = -1;
        }

        public void TransitionTo(int index, float enterTime = -1)
        {
            ...
            m_timeToNext = (enterTime < -0.5f ? GetTargetEnterTime(m_targetIndex) : (enterTime < 1e-6f ? 0.1f : enterTime)) * (1f - GetWeight(m_targetIndex));
        ...
        private float ModifyWeight(int index, float delta)
        {
            if (index < 0 || index >= inputCount)
                return 0;
            float weight = Mathf.Clamp01(m_animMixer.GetInputWeight(index) + Mathf.Clamp(delta, -0.1f, 0.1f));
            m_animMixer.SetInputWeight(index, weight);
            return weight;
        }
    }
